...
class Flash_sbWoo {
    ...
    **/
	...
	switch ( $position_ref ){
		...
		break;
	    case '2':
		add_action( 'woocommerce_before_shop_loop_item_title', array($this, 'woo_fs_pro_add_c'), 12 );
		add_action( 'woocommerce_single_product_summary', array($this, 'woo_fs_pro_add_l'), 6 );
		break;
	    case '3':
		add_action( 'woocommerce_before_shop_loop_item_title', array($this, 'woo_fs_pro_add_c'), 12 );
    ...
    **/
	...
	global $product;
        ...
        if (isset($ctd)){
    		...
    		echo '<div class="woo_flash_sales" data-id="'.esc_html($ctd).'"></div>';
    	    }
    	}
    }
    ...
    **/
	...
	if( isset(get_option( 'woofspro_options' ) ['textsale_status']) ){
	    if ( get_option( 'woofspro_options' ) ['textsale_status'] ) {
		echo '<div align="center" style="font-family:'. esc_html( $this->woo_fs_pro_get_sale_text_font() ) .';color:'. esc_html( $this->woo_fs_pro_get_sale_text_color() ).';font-size:' .esc_html( $this->woo_fs_pro_get_sale_text_font_size() ). 'px;margin-bottom:0px;border:'.$this->woo_fs_pro_get_sale_text_border().'px solid '.esc_html( $this->woo_fs_pro_get_sale_text_color() ). ';">'.get_option( 'woofspro_options' ) ['sale_text'].'</div>';
	    }
	}
    }
    ...
    **/
	...
	if( isset(get_option( 'woofspro_options' ) ['textsale_status']) ){
	    if ( get_option( 'woofspro_options' ) ['textsale_status'] ) {
		echo '<div align="left" style="padding-left:15px;font-family:' .esc_html( $this->woo_fs_pro_get_sale_text_font() ) .';color:' .esc_htnl( $this->woo_fs_pro_get_sale_text_color() ). ';font-size:'.$this->woo_fs_pro_get_sale_text_font_size().'px;margin-bottom:0px;border:'.$this->woo_fs_pro_get_sale_text_border().'px solid '.$this->woo_fs_pro_get_sale_text_color().';">'.get_option( 'woofspro_options' ) ['sale_text'].'</div>';
	    }
	}
    }
    ...
    **/
	...
	echo '<style id="storefront-style-inline-css" type="text/css">
	    .woo_flash_sales {
		border-width: '.esc_html( $this->woo_fs_pro_get_border_top() ).'px '.$this->woo_fs_pro_get_border_right().'px '.$this->woo_fs_pro_get_border_bottom().'px '.$this->woo_fs_pro_get_border_left().'px;
    		border-style: solid;
    		background: '.esc_html( $this->woo_fs_pro_get_background() ).';
    		color: '.esc_html( $this->woo_fs_pro_get_color() ).';
    		padding: 2px;
    		border-color: '.esc_html($this->woo_fs_pro_get_color()).';
		...
		margin-left: 5px;
    		min-width: 40px;
    		text-align: center;
		font-size: '.esc_html( $this->woo_fs_pro_get_dhms() ).'px;
		font-weight: normal;
    	    }.woo_flash_sales span:first-child {margin-left: 0;}.woo_flash_sales span span {font-size: '.esc_html( $this->woo_fs_pro_get_font_size() ).'px;display: block;}</style>';
    }
    ...
    **/
	...
	add_menu_page(
	    esc_html__( 'BoostSales ', 'woofspro' ),
	    esc_html__( 'BoostSales ', 'woofspro' ).$n,
	    'manage_options',
	    'woo-fs-pro',
    ...
    **/
	...
	if( isset( $_GET['p'] ) ){
	    $apage = filter_var( $_GET['p'], FILTER_SANITIZE_NUMBER_INT );
	}else{
	    $apage = 1;
	}
	?>
	    <div class="wrap">
	    	    <h2>Boost Online Sales</h2><hr/>
		    <?php
			settings_errors();
			switch($apage){
			    case 1:
				...
				echo '<div>'. esc_html( $admin->woo_fs_pro_settings() ) . '</div>';
				break;
			    case 2:
				echo '
				<ul class="wp-tab-bar">
				...
				echo '<div>'. esc_html( $admin->woo_fs_pro_about() ). '</div>';
				break;
			    case 3:
				echo '
				<ul class="wp-tab-bar">
				...
				echo '<div>'. esc_html($admin->woo_fs_pro_tutorial() ). '</div>';
				break;
			}
		    ?>
    	    </div>
    ...
    **/
	...
	<div class="wp-tab-panel" style="min-height:730px;">
	    <h1><?php esc_html_e( 'Setup Boost Online Sales', 'woofspro' ); ?></h1><hr />
	    <?php
	    if ( !strlen(get_option( 'woofspro_options' )['fontsize'] ) ){
		...
		<tr>
			...
			<?php $value = self::get_theme_option( 'fontsize' ); ?>
			    ...
			    <?php
			        $size_options = array(
			    	'5'  => esc_html__( '5px', 'woofspro' ),
			    	'6'  => esc_html__( '6px', 'woofspro' ),
		    ...
		    <th scope="row"><?php esc_html_e( 'Days,Hours... Font Size', 'woofspro' ); ?></th>
		    <td>
		...
		<tr valign="top">
			...
			<td>
			    ...
			    <?php
			        ...
			        '3' => esc_html__( 'Extr: Border = 3', 'woofspro' ),
			        '4' => esc_html__( 'Mega: Border = 4', 'woofspro' ),
				);
				foreach ( $size_options as $ids => $label ) { ?>
				<option value="<?php echo esc_attr( $ids ); ?>" <?php selected( $value, $ids, true ); ?>>
				    <?php echo esc_html( $label ); ?>
		...
		<tr>
			...
			<td>
			    ...
			    <?php
			        ...
			        'Impact' => esc_html__( 'Impact', 'woofspro' ),
			        'Montserrat' => esc_html__( 'Montserrat', 'woofspro' )
				);
				foreach ( $size_options as $ids => $label ) { ?>
				<option value="<?php echo esc_attr( $ids ); ?>" <?php selected( $value, $ids, true ); ?>>
		...
		<tr valign="top" class="FSPro-custom-admin-screen-background-section">
			...
			<td>
			    ...
			    <?php
			        $size_options = array(
			        '3' => esc_html__( 'Above product title', 'woofspro' ),
			        '1' => esc_html__( 'Under price(s)', 'woofspro' ),
			    	'2' => esc_html__( 'Under title and prices', 'woofspro' ),
				);
				foreach ( $size_options as $ids => $label ) { ?>
				<option value="<?php echo esc_html( $ids ); ?>" <?php selected( $value, $ids, true ); ?>>
				    <?php echo esc_html( $label ); ?>
				</option>
			    <?php } ?>
			    </select>
    ...
    **/
	...
	<h1><?php esc_html_e( 'About Boost Online Sales', 'woofspro' ); ?></h1><hr />
	<div><br />
	<h3>Boost Online Sales is a free plugin that dramatically boost your sales.</h3>
	<br />
	<h2>This plugin is dedicated to Vic & Rox</h2>
	<br />
	<h2><a href="http://www.app.ro/wordpress-plugins/" target="_blank">Boost Online Sales webpage!</h3></a>
	</div>
	</div>
	<?php
    }
    ...
    **/
	...
	?>
	<div>
	<h1><?php esc_html_e( 'Tutorial Boost Online Sales', 'woofspro' ); ?></h1><hr />
	<h2>Image capture - Demo images </h2>
	<img src="<?php echo BOOST_SALE_WOOCOMMERCE_SRC.'/images/shop.jpg';?>">
	<h2>Product page capture of Boost Online Sales</h2>
	<img src="<?php echo BOOST_SALE_WOOCOMMERCE_SRC.'/images/product.jpg';?>">
	<img src="">
	</div>
	<?php
    }
    ...
    **/
    public function woo_fs_pro_style_cs(){
        wp_register_style ( 'wpstyle' , plugins_url (BOOST_SALE_WOOCOMMERCE_SRC.'/css/woofspro.min.css' ) );
    ...
    **/
	...
	wp_register_script( 'woofsjs', BOOST_SALE_WOOCOMMERCE_SRC.'/js/bfs.min.js' );
	$woofstr = array(
	    'fsobjd'		=> __( 'Days','woofspro' ),
	    'fsobjh'		=> __( 'Hrs' ,'woofspro' ),
	    'fsobjm'		=> __( 'Min' ,'woofspro' ),
	    'fsobjs'  		=> __( 'Sec' ,'woofspro' ),
	);
